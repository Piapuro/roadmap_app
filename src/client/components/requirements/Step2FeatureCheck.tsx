...
import { FEATURE_LIST } from '@/client/constants/featureList'
import { PRODUCT_TYPE_LABELS } from '@/types/requirements'

export function Step2FeatureCheck() {
  const { productType, selectedFeatures, toggleFeature } = useWizardStore()
  const [search, setSearch] = useState('')

  if (!productType) return null

  const categories = FEATURE_LIST[productType]
    ...
    <div className="flex flex-1 min-h-0">
      ...
      <div className="flex-1 flex flex-col gap-5 p-8 overflow-y-auto bg-[#F2F3F0]">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-full bg-[#FF8400] px-3 py-1 text-xs font-semibold text-[#111111] font-[family-name:var(--font-jetbrains-mono)]">
      ...
      <aside className="w-[340px] shrink-0 flex flex-col bg-white border-l border-[#CBCCC9]">
        <div className="flex flex-col gap-1 px-6 py-6 border-b border-[#CBCCC9] shrink-0">
          ...
          <p className="font-[family-name:var(--font-geist-sans)] text-[#666666] text-xs">
            {checkedCount} / {allFeatures.length} 機能を選択中
          </p>
          <div className="mt-2 h-1.5 w-full rounded-full bg-[#CBCCC9] overflow-hidden">
            <div
              ...
              style={{ width: `${allFeatures.length ? (checkedCount / allFeatures.length) * 100 : 0}%` }}
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-[#CBCCC9]">
          {selectedFeatures.map((feature) => (
            <div key={feature} className="flex items-center gap-2 px-5 py-2.5">
